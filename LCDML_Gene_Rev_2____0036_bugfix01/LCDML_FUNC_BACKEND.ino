...
void LCDML_BACK_setup(LCDML_BACKEND_position_drum) {
  // Check if drum is positioned correctly
  if (digitalRead(rotEndSw) == 1) {
    parkRoaster();
    LCDML_lcd_menu_display();
    LCDML_BACK_stopStable(LCDML_BACKEND_position_drum);
  }
}
boolean LCDML_BACK_loop(LCDML_BACKEND_position_drum)
{
  return false;
}
void LCDML_BACK_stable(LCDML_BACKEND_position_drum) {}
